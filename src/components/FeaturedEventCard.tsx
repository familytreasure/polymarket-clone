  ...
  return (
    ...
    >
      <div className="p-6 flex flex-col gap-5">
        ...
        <div className={`flex items-center gap-6 rounded-xl bg-pm-bg/60 px-4 py-3 border border-pm-border ${flashClass}`}>
          <div className="flex flex-col items-center gap-0.5">
            <span className={`text-3xl font-bold tabular-nums ${isBullish ? 'text-pm-green' : 'text-pm-red'}`}>
              {Math.round(yesPrice * 100)}¢
            </span>
            <span className="text-xs text-pm-muted">{primaryMarket.outcomes[0] ?? 'Yes'}</span>
          </div>
          ...
          <div className="flex flex-col items-center gap-0.5">
            ...
            <span className="text-xs text-pm-muted">{primaryMarket.outcomes[1] ?? 'No'}</span>
          </div>
        </div>
